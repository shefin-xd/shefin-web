...
openAdmin.addEventListener("click", async () => { const response = await fetch(apiUrl("/admin/dashboard"), { credentials: "include" }); status.textContent = response.ok ? "Dashboard API access confirmed." : "Your admin session could not be confirmed."; });
